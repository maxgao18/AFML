...
def _ou_process(
    ...
    a = np.random.normal(mu, var, n_samples)
    a[0] = 0
...
def make_randomt1_data(
    n_samples: int = 10000, max_days: float = 5.0, Bdate: bool = True
):
...
def make_classification_data(
    n_features=40, n_informative=10, n_redundant=10, n_samples=10000, days: int = 1
):
    # generate a random dataset for a classification problem
    _today = dt.datetime.today()
    ...
    df0 = ["I_%s" % i for i in range(n_informative)] + [
        "R_%s" % i for i in range(n_redundant)
    ]
...
def create_portfolio(
    price: list = [95, 1000],
    position: list = [1000, 10000],
    n_sample: int = 10000,
    imbalance: bool = True,
):
    df = make_randomt1_data(n_samples=n_sample, max_days=10.0, Bdate=True)
    ...
    df["label"] = np.nan
    if imbalance:
        for idx in df[df["open"] < 200].index:
            df.loc[idx, "label"] = 1
    else:
        for idx in df[df["open"] < df["open"].quantile(0.4)].index:
            df.loc[idx, "label"] = 1
    ...
    p(
        "\nJunk bond (Below BBB-grade): {0} %\nDividend equity: {1} %".format(
            100 * junk_bond / n_sample, 100 * div_equity / n_sample
        )
    )
    return df
...
def create_rtndf(
    ...
    df = pd.DataFrame(1.0, index=idx, columns=np.arange(n_samples))
